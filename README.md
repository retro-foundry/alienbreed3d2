...
samples present smoothly at the host frame rate. Native direct play follows
the first port's `display_init` desktop presentation: it queries the active
desktop bounds and creates a borderless normal OpenGL window there, without an
SDL fullscreen request or monitor-mode change, then renders directly to the
complete SDL drawable. Its main
