 ...
 *
 * The ray tracer uses it only from the second bounce onward. The primary hit
 * stays fully path traced -- emitters, ray-traced shadows, the lot -- and a
 * bounce ray that lands on a surface reads this instead of spawning the rays
 * it would take to light that surface properly. Dark areas are lit almost
 * entirely by those bounces, so that is where a path tracer has the least
 * signal and the most grain; a deterministic value there removes the grain
