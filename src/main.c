...
static int game_app_run_gpu_smoke(GameApp *app)
{
    ...
    for (uint16_t level_index = first_level; level_index <= last_level; ++level_index) {
        ...
        if (smoke_backend == RENDERER_BACKEND_RTX) {
             ...
             * sequence and require the uploaded emission scales to differ, or
             * to stay put with rtx_emissive_animation off. The assertion is on
             * the scene the backend uploaded rather than on the presented
             * image: a fresh-sample path tracer's frame-to-frame delta is far
             * larger than one pulsing panel until the reconstruction settles,
             * so an image comparison would prove nothing here. Levels that
             * animate no Gouraud brightness leave every word alone and say so
             * instead.
             */
            {
                const RendererRayTracingOptions *ray_tracing =
                    &app->desktop_settings.ray_tracing;
                const int emissive_animation =
                    ray_tracing->emissive_animation_set != 0u ?
                        ray_tracing->emissive_animation != 0u :
                ...
                if (dim_point_fold == bright_point_fold) {
                    fprintf(stdout,
                            ...
                            (char)('A' + level_index));
                } else if (!emissive_animation) {
                    if (dim_emissive_fold != bright_emissive_fold) {
                        fprintf(stderr,
                                "[RENDER] DXR Level %c authored emission followed "
                                "the source Gouraud brightness with "
                    ...
                    fprintf(stdout,
                            "[RENDER] DXR Level %c emission held steady: "
                            "rtx_emissive_animation off\n",
                            (char)('A' + level_index));
