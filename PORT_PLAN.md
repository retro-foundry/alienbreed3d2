...
- [x] `objdrawhires.s:doapoly` vector polygons retain their authored point
  ...
  the first three authored points select or reject the entire polygon once,
  before its GPU fan is submitted, rather than culling the generated triangles
  independently or applying a global winding rule to the distinct wall and
  object source formats. Additive `predoglare` faces are a deliberate
  double-sided presentation exception: both authored windings are submitted
  with depth writes disabled. The all-level hidden GPU smoke forces a
...
- [x] Bitmap presentation no longer applies a generic RGB room-light multiplier
  after source palette resolution. `objdrawhires.s:draw_Bitmap` selects its
  normal bitmap's 32-colour row from `ObjT_Brightness + (depth >> 6)`;
  `draw_bitmap_lighted` and the additive/glare modes retain their own
  source-derived palette or blend result. Cached normal-frame variants include
  that source palette-row identity.
...
- [x] Native direct play copies Alien Breed 3D I `src/display.c:display_init`:
  ...
  The desktop presenter uses SDL's measured normal-window borders to shift the
  frame past the top/left bounds and grow the client by that same inset,
  preserving bottom/right overscan and taskbar coverage without changing the
  window style or monitor mode.
...
- [x] `src/level_draw_graph.*` follows the maintained
  ...
  edge, the direct `LiftRoutine` boundary owns the shared extent and the
  static segment is clipped only at that boundary. Reverse-directed records
  remain the independent authored material for the opposite side. This
  preserves the moving-panel illusion without per-face height normalization,
  depth bias, separated counterpart surfaces, or segmented GPU geometry.
